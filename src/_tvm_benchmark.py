...
class CarvanaDataset(Dataset):
    ...
    def __init__(self, images_dir, masks_dir, scale=0.5):
    ...
    @staticmethod
    def _preprocess(pil_img, scale, is_mask):
        w, h = pil_img.size
        new_w, new_h = int(w * scale), int(h * scale)
        pil_img = pil_img.resize((new_w, new_h), resample=Image.NEAREST if is_mask else Image.BICUBIC)
        if is_mask:
            arr = np.asarray(pil_img)
            out = np.zeros((new_h, new_w), dtype=np.int64)
            for idx, value in enumerate(CarvanaDataset.MASK_VALUES):
                out[arr == value] = idx
            return out
    ...
    def __getitem__(self, idx):
        ...
        img = self._preprocess(img, self.scale, is_mask=False)
        mask = self._preprocess(mask, self.scale, is_mask=True)
        return torch.from_numpy(img), torch.from_numpy(mask)


def get_dataloader(data_dir, batch_size, num_workers, img_scale, val_fraction=0.1, repeat=1):
    """Create validation DataLoader.

    Args:
        repeat: Number of times to repeat the dataset. Default is 1 (no repeat).
            For TVM benchmarking, a single pass is sufficient since we control
            the number of batches via --max-batches.
    """
    imgs_dir = Path(data_dir) / "imgs"
    masks_dir = Path(data_dir) / "masks"
    full_ds = CarvanaDataset(imgs_dir, masks_dir, scale=img_scale)
    n = len(full_ds)
    n_val = max(1, int(n * val_fraction))
    val_ds, _ = random_split(full_ds, [n_val, n - n_val], generator=torch.Generator().manual_seed(42))
    # Only wrap in ConcatDataset if repeat > 1
    if repeat > 1:
        from torch.utils.data import ConcatDataset
        val_ds = ConcatDataset([val_ds] * repeat)
...
def main():
    ...
    # Step 2: Load data (no repeat — max_batches controls benchmark length)
    print("\n── Loading data ──")
    loader = get_dataloader(args.data_dir, args.batch_size, args.num_workers, args.img_scale, repeat=1)
