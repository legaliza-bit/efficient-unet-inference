    ...
    def __getitem__(self, idx: int):
        ...
        if not img_files:
            raise FileNotFoundError(
                f"No image found for id '{name}' in {self.images_dir}"
            )
        if not mask_files:
            raise FileNotFoundError(
                f"No mask found for id '{name}' in {self.masks_dir}"
            )
...
def get_carvana(
    split: str = "train", val_fraction: float = 0.1, scale: float = IMG_SCALE
):
...
def prepare_benchmark_cache(
    ...
    indices = torch.randperm(len(ds), generator=torch.Generator().manual_seed(42))[
        :n_samples
    ].tolist()
